...
(function(window, Calendar) {
    ...
    cal.on({
        ...
        'beforeUpdateSchedule': function(e) {
			...
			updateStorageScheduleList(e.schedule);
        },
        'beforeDeleteSchedule': function(e) {
            console.log('beforeDeleteSchedule', e);
            cal.deleteSchedule(e.schedule.id, e.schedule.calendarId);
			deleteStorageScheduleList(e.schedule);
        },
    ...
    function saveNewSchedule(scheduleData) {
		...
		// TODO parksungho
		var ss = localStorage.schedule ? JSON.parse(localStorage.schedule) : [];
...
// TODO parksungho
	...
	// TODO parksungho
	function updateStorageScheduleList(schedule) {
		if (localStorage.schedule) {
			var ss = JSON.parse(localStorage.schedule);
			for (var i in ScheduleList) {
				if (ScheduleList[i].id == schedule.id) {
					ScheduleList[i].calendarId = schedule.calendarId;
					ScheduleList[i].title = schedule.title;
					ScheduleList[i].location = schedule.location;
					ScheduleList[i].start = schedule.start._date;
					ScheduleList[i].end = schedule.end._date;
					ScheduleList[i].isAllDay = schedule.isAllDay;
					ScheduleList[i].category = schedule.isAllDay ? 'allday' : 'time'
					if (schedule.isAllDay) {
						ScheduleList[i].start.setHours(9);
					}
					ScheduleList[i].state = schedule.state;
					ss[i] = ScheduleList[i];
					localStorage.schedule = JSON.stringify(ss);
				}
			}
	...
	function deleteStorageScheduleList(schedule) {
		if (localStorage.schedule) {
			var ss = JSON.parse(localStorage.schedule);
			for (var i in ss) {
				if (ss[i].id == schedule.id) {
					ss.splice(i, 1);
					localStorage.schedule = JSON.stringify(ss);
					return;
				}
	...
	function getStorageScheduleList() {
		if (localStorage.schedule) {
			ScheduleList = [];
			var ss = JSON.parse(localStorage.schedule);
			for (var i in ss) {
				ScheduleList.push(ss[i]);
			}
		}
	}
	// TODO parksungho
